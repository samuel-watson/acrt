...
find_optimal_designs <- function(z1_vec, design_grid, model_summaries,
  ...
  # Continue only if not stopping for efficacy or futility
  ...
  # Add resource variables from model summaries
  if (!is.null(resource_vars)) {
    for (var in resource_vars) {
...
compute_power_and_expectations <- function(design_grid, model_summaries, I1_eff,
  ...
  # Expected resources (only over continuation region)
  expected <- list()
  design_cols <- names(design_grid)
  for (col in design_cols) {
    if (col %in% names(opt_designs)) {
      vals <- opt_designs[[col]]
      vals[is.na(vals)] <- 0
      expected[[col]] <- sum(vals[continue] * weights[continue])
    }
  }

  # Add resource vars if specified
  if (!is.null(resource_vars)) {
    for (var in resource_vars) {
      if (var %in% names(opt_designs)) {
        vals <- opt_designs[[var]]
        vals[is.na(vals)] <- 0
        expected[[var]] <- sum(vals[continue] * weights[continue])
      }
    }
  }
  ...
  list(
    ...
    quadrature = list(
      ...
      optimal_designs = opt_designs  # Make sure this is here!
    ),
    params = list(
      mu1 = mu1,
      b1 = b1
...
find_lambda_for_power <- function(target_power = 0.8,
  ...
  # Check maximum achievable power (lambda -> 0, always continue with max design)
  ...
  # Check minimum power (lambda -> large, stop early for futility)
  if (verbose) cat("Checking minimum power (lambda large)...\n")
  ...
  for (iter in 1:max_iter) {
    # Bisect on log scale for better behaviour
    ...
    if (verbose) {
      cat(sprintf("Iter %2d: lambda = %.4e, power = %.4f (target = %.4f)\n",
