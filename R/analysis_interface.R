...
plot.adaptive_results <- function(x, type = c("EN", "Nmax", "power", "probabilities", 
  ...
  if (x$explored) {
    # Exploration plots
    if (type == "decision") {
      # Show decision rules for first converged design
      idx <- which(x$raw$summary$converged)[1]
      if (!is.na(idx) && !is.null(x$raw$results[[idx]]$results)) {
        plot_decision_rules(x$raw$results[[idx]]$results, ...)
      } else {
        message("No converged results available for decision plot")
      }
    } else if (type == "pareto") {
      # Pareto frontier
      pareto <- find_constrained_pareto(x$raw$summary, 
                                        objectives = list(E_N = "min", N_max = "min"))
      plot_pareto(pareto, all_designs = x$raw$summary, ...)
    } else {
    ...
    # Single design plots
    if (type %in% c("decision", "EN", "Nmax")) {
      if (!is.null(x$raw$results)) {
        plot_decision_rules(x$raw$results, ...)
      } else {
...
print.adaptive_comparison <- function(x, ...) {
  ...
  # Print fixed design info (handle different column names)
